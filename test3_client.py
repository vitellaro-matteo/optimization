...
class Client(threading.Thread):
    ...
    def plot_accuracy_graph(self):
        if self.id == 1:
            ...
            first_key = next(iter(accuracy_dict))
            ...
            for label in accuracy_dict:
                if label != "Aggregation":
                    ...
                    print("Self.leaders")
                    print(self.leaders)
                    # Calculate mean of the first key's value list
                    aggregation_values = [np.mean(accuracy_dict[first_key][i:i + 1]) 
                                        for i in range(0, len(accuracy_dict[first_key]), 1)]
