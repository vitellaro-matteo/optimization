...
class Client(threading.Thread):
    ...
    def aggregate_leader_models(self):
        if self.is_leader:
            ...
            average_accuracy = self.evaluate_model(aggregated_weights, 0)
            safe_print("Average accuracy after leader aggregation: ", average_accuracy, " Leader: ", self.leader.id)
    ...
    def aggregate_and_update_global_model(self):
        if self.is_leader:
            ...
            accuracy_dict[self.id].append(accuracy)
            self.local_model = aggregated_weights
            safe_print("Accuracy: ", accuracy, " Cluster with Leader: ", self.leader.id)
